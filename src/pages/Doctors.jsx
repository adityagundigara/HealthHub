...
const Doctors = () => {
  ...
  return (
    <div className={`text-[3vw] flex flex-col items-start sm:text-[2vw] lg:text-[1vw]`}>
      ...
      <div className={`${filterDoc.length === 0
        ? 'flex justify-between w-[100%] gap-8'
        : 'w-full flex justify-between gap-2'
        }`} >
        <div className={`flex flex-col gap-3 text-center cursor-pointer`}>
          <p onClick={() => { speciality === 'General physician' ? [navigate('/doctor'), console.log(speciality)] : [navigate('/doctor/General physician'), console.log(speciality)] }} className={`border border-gray-500 rounded-sm px-4 py-1 sm:px-3 sm:py-1 md:px-6 lg:px-4 ${speciality === 'General physician' ? "bg-indigo-50" : ""}`}>General Physician</p>
        ...
        <div className={`${filterDoc.length === 0
          ...
          : 'grid grid-cols-1 gap-y-8 gap-x-2 sm:grid-cols-2 sm:w-[55vw] sm:justify-items-end lg:w-[80%] lg:gap-x-6 lg:gap-y-8 lg:grid-cols-4 lg:justify-items-end'
          }`}>
          {
            filterDoc.length === 0 ? (
              <p className='text-[4vw] text-center sm:text-[3vw] lg:text-[2.5vw]'>No doctors available for this specialty</p>
            ) : (
              filterDoc.map((item, index) => (
                <div key={index}
                  onClick={() => (navigate(`/appointment/${item._id}`))}
                  className={'w-[50vw] flex flex-col items-center justify-center bg-gray-200 rounded-lg border-gray-300 border-[1px] cursor-pointer sm:w-[25vw] lg:w-[15vw] xl:w-[14vw]'}>
                  <img className={'w-[350px] sm:w-[40vw] lg:w-[150px] xl:w-[20vw]'} src={item.image} alt="" />
                  <div className={'bg-white w-full p-3 flex flex-col items-start sm:p-4 flex-grow'}>
                    ...
                    <div className={'flex-grow'}>
                      <p className={'text-[4vw] font-medium mb-1 sm:mb-0 sm:mt-2 xl:text-sm sm:text-[2vw] lg:text-[1.2vw] xl:text-[1vw] 2xl:text-[1.2vw] 2xl:mb-2'}>
                        {item.name}
                      </p>
                      <p className={'text-gray-500 text-sm lg:text-[1vw] 2xl:w-[1vw] xl:text-[1vw]'}>{item.speciality}</p>
                    </div>
                  </div>
                </div>
              ))
